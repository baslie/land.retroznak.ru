...
export function ConsultationModal() {
  ...
  useEffect(() => {
    const checkAndOpenModal = () => {
      ...
      // Support main hash and aliases
      if (hash === "#consultation" || hash === "#timeline-form" || hash === "#custom-project") {
        setIsOpen(true);
      }
    };
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // Clear hash when closing (including aliases)
      const hash = window.location.hash;
      if (hash === "#consultation" || hash === "#timeline-form" || hash === "#custom-project") {
    ...
    >
      ...
      >
        ...
        <div onClick={(e) => e.stopPropagation()}>
          <ConsultationForm className="!border-0 !bg-transparent !p-0 !shadow-none" />
        </div>
      </div>
    </div>
  );
